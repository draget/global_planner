...
double Maneuver::J(const Maneuver &previous,
  ...
  double _C_k = C_k();
  double _C_c = C_c(previous);
  double _C_s = C_s(maneuvers);

  return w_s * _C_s + w_k * _C_k + w_c * _C_c;
}
...
double Maneuver::C_k() const {
  ...
  // This is much faster than default of 10.
  size_t max_refinements = 5;
  double integral = trapezoidal(l, s_i(), s_f(),  1e-4, max_refinements);
...
double Maneuver::C_c(const Maneuver &previous) const {
  ...
  size_t max_refinements = 5;
  double result = 1 / (s_2 - s_1) * trapezoidal(l, s_1, s_2,  1e-4, max_refinements);

...
DEFINE_double(velocity, 5, "velocity of car");
DEFINE_double(frequency, 5, "update frequency");
...
/*  double L = 316;
  ...
  for (double i = 0; i < L; i++) {
    waypoints.push_back(std::make_pair(i, 0));
  }
*/
  ...
  std::cout << "Placed " << cone_count << " cones." << '\n';
  std::cout << "Baseframe length: " << jack.baseframe().length() << '\n';
